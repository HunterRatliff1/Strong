...
seasons$Coach.es. <- sapply(seasons$Coach.es., function(l) {
  ...
  if(grepl("Mack", l)) return("Mack Brown")
  else(return(NA))
})
...
seasons %>%
  ...
  geom_point(aes(color=Coach, shape=Coach, size=SOS))+
  geom_line(aes(group=Coach), alpha=0.4) +
  geom_smooth() +
...
strong %>%
  ...
  labs(x="Strength of Schedule", y="W/L Percentage", title="Win/Loss in Respect To S.O.S.") + 
  ylim(c(0,1)) + theme_ut
